...
> **2026-09-21 상태: 코드는 출시 준비 끝.** 로그인 없는 버전(주보·말씀노트·성경·나눔(기기 저장)·교회).
> Supabase 없이 빌드됩니다 — eas.json 에 env 없음. 주보를 매주 서버로 밀려면 나중에 §1 을 하고 eas.json 에 env 를 추가.
> 남은 건 Jay 가 직접: **App Store Connect 앱 생성 → ascAppId 입력 → `eas build` → `eas submit` → 스토어 문구·스크린샷 입력.**
